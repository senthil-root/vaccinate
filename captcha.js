...
const {hashes_letters}    = require('./constants');
...
filenames.forEach((file) => {
    Jimp.read(`./letters/${file}`).then(image => {
        ...
        // console.log(file + ':' + LetterHash);
        lettersHashesMap.set(file, LetterHash);
        hashes_letters[file] = LetterHash;
    })
});

const svgData      = fs.readFileSync('./capcha.svg');
...
for (let pos = 0; pos < letters.length; pos++) {
    ...
    fs.writeFile(`letter_${pos}.svg`, letterSVG, function (err) {
        if (err) throw err;
        sharp(`letter_${pos}.svg`)
            .sharpen()
            .normalise()
            .negate()
            .extend({
